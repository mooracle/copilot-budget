...
- **commitHook.ts** — Installs/uninstalls a POSIX `prepare-commit-msg` hook that reads `copilot-budget` from the git dir and appends git trailers (`AI-Premium-Requests`, `AI-Est-Cost`, `AI-Model`) to commit messages. All exported functions (`isHookInstalled`, `installHook`, `uninstallHook`) are async. Uses `resolveGitCommonDir` (with `vscode.Uri`) and `readTextFile`/`writeTextFile` from `fsUtils`. Includes a `makeExecutable` helper that uses `fs.chmodSync` for local files and a VS Code shell task for remote filesystems. The hook script uses `git rev-parse --git-dir` at runtime for worktree-safe tracking file resolution. Resets the tracking file after appending.
