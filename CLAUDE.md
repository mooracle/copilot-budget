...
- **trackingFile.ts** — Writes stats to `<gitdir>/copilot-budget` in key=value format for the commit hook to read. Uses `resolveGitDir` so each worktree gets its own tracking file. Includes `PREMIUM_REQUESTS`, `ESTIMATED_COST`, and per-model premium request data.
...
- **gitDir.ts** — Shared utility for resolving git directories. `resolveGitDir(workspaceRoot)` follows `.git` files (worktrees, submodules, devcontainers) to the real git dir. `resolveGitCommonDir(workspaceRoot)` further follows the `commondir` file in worktrees to reach the shared git dir (where hooks live).
- **commitHook.ts** — Installs/uninstalls a POSIX `prepare-commit-msg` hook that reads `copilot-budget` from the git dir and appends git trailers (`AI-Premium-Requests`, `AI-Est-Cost`, `AI-Model`) to commit messages. Uses `resolveGitCommonDir` so hooks are installed in the shared git dir (correct for worktrees). The hook script uses `git rev-parse --git-dir` at runtime for worktree-safe tracking file resolution. Resets the tracking file after appending.
