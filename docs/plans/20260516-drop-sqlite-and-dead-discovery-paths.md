...
- [x] remove `"sql.js": "^1.14.0"` from `dependencies` in `package.json`
- [x] run `npm install` to refresh `package-lock.json`
- [x] delete the `copyWasm()` function and both call sites (build + watch) in `esbuild.js`
- [x] run `npm run compile` and confirm `dist/sql-wasm.wasm` is absent (delete it manually if a stale copy lingers from a prior build)
- [x] run `npm test && npm run lint` — must pass before next task
